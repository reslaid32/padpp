...
extern "C"
{
  ...
  _PADPP_PUBLIC void
  padpp_destroy_iso9797_m1(padpp_handle_t ctx)
  {
    delete static_cast<ISO9797_M1_BytePadder*>(ctx);
  }

  _PADPP_PUBLIC int
  padpp_pad_iso9797_m1(padpp_handle_t ctx, uint8_t* dst, size_t* dst_len,
  ...
  _PADPP_PUBLIC int
  padpp_unpad_iso9797_m1(padpp_handle_t ctx, uint8_t* dst, size_t* dst_len,
                         const uint8_t* src, size_t src_len, size_t block_size)
  {
    if (!ctx || !dst || !dst_len || !src)
      return -1;
